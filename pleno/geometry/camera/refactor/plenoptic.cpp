...
const MicroLensesArray& PlenopticCamera::mla() const { return _mla; }
MicroLensesArray& PlenopticCamera::mla() { return _mla; }

const MicroImagesArray& PlenopticCamera::mia() const { return _mia; }
MicroImagesArray& PlenopticCamera::mia() { return _mia; }     

const InternalParameters& PlenopticCamera::params() const { return _params; }
InternalParameters& PlenopticCamera::params() { return _params; }   

double PlenopticCamera::distance_focus() const { return _dist_focus; } 
double& PlenopticCamera::distance_focus() { return _dist_focus; }   

const ThinLensCamera& PlenopticCamera::main_lens() const { return _main_lens; }
ThinLensCamera& PlenopticCamera::main_lens() { return _main_lens; }

const Distortions& PlenopticCamera::main_lens_distortions() const { return _distortions; }
Distortions& PlenopticCamera::main_lens_distortions() { return _distortions; }
...
{		
	...
	const double theta_z 	= get_rotation_angle(_mia.pose().rotation()); //std::atan2( _mia.pose().rotation()(1, 0), _mia.pose().rotation()(0, 0) );
	...
	//set focal lengths
	_mla.init(I); for(std::size_t i=0; i<I; ++i ) mla().f(i) = (1. / params().c_prime[i]) * params().kappa_approx * (d / 2.); 
...
bool PlenopticCamera::project_through_micro_lens(const P3D& p, std::size_t k, std::size_t l, P2D& projection) const
{
    ...
    P3D Ckl_cam = from_coordinate_system_of(mla().pose(), mla().node(k,l)); //_mla.nodeInWorld(k,l);
    Ray3D ray;
